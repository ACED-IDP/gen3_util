...
@click.group(cls=g3t.NaturalOrderGroup)
def cli():
...
def add(ctx, target):
    ...
    """
    from g3t.git.adder import add_file, add_url
...
@click.option('--transfer-method',
              type=click.Choice(g3t.FILE_TRANSFER_METHODS.keys()),
              default='gen3',
              show_default=True,
...
def push(ctx, step: str, transfer_method: str, overwrite: bool, re_run: bool, wait: bool, dry_run: bool):
    ...
    try:
        with Halo(text='Scanning', spinner='line', placement='right', color='white'):
            ...
            auth = g3t.config.ensure_auth(config=config)
...
@cli.command()
@click.argument('project_id', default=None, required=False, envvar=f"{g3t.ENV_VARIABLE_PREFIX}PROJECT_ID", metavar='PROJECT_ID')
@click.pass_obj
...
def download_snapshot(auth, config):
    ...
    from g3t.git.cloner import find_latest_snapshot
...
def ping(config: Config):
    ...
    with CLIOutput(config=config) as output:
        ...
        gen_client_ini_file = g3t.config.gen_client_ini_path()
        auth = None
