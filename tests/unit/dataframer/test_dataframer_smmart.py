...
def smmart_local_db(smmart_fixture_path: Path) -> LocalFHIRDatabase:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert smmart_fixture_path.exists(), f"Fixture path {smmart_fixture_path.absolute()} does not exist."
...
def test_observations(smmart_fixture_path, smmart_local_db, smmart_observation_dataframes):
    ...
    assert len(smmart_observation_dataframes) == num_observation, "observation ndjson metadata and expected observation dataframes are not the same length, check that the fixture have the same number of rows as the metadata"
    ...
    for expected, actual in zip(smmart_observation_dataframes, actual_dataframes):
        assert expected == actual, f"Observation differs than expected, use pytest -vv flag for diff"